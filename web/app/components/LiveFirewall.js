...
function outcomeLabel(event) {
  ...
  const approval = String(event?.approval_status || "").toLowerCase();
...
export default function LiveFirewall() {
  ...
  async function approve(decision) {
    if (!result?.event_id) {
      console.log("No event_id on result:", result);
      return;
    }

    setResult((prev) => ({ ...prev, decisionMade: decision }));

    try {
      const res = await fetch(`${API_URL}/events/${result.event_id}/approve`, {
        method: "POST",
      ...
      console.log("Approve request failed:", err);
    }
  }
  ...
  const resultState = useMemo(() => {
    ...
    if (
      type.includes("pause") ||
      String(result.manager_status || "").includes("paused")
    ) {
      return {
        ...
        {resultState && (
          ...
          >
            <div className="p-5 md:p-7">
              ...
              {resultState.color === "warning" && !result.decisionMade && (
              ...
              {result.decisionMade && (
                <div className="mt-5 font-mono text-xs text-neutral-500">
                  Decision recorded: {result.decisionMade.toUpperCase()}
                </div>
              )}
            </div>
          </motion.div>
